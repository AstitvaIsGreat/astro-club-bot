...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (subcommand === 'delete') {
            ...
            if (!isHost && !hasManagerPerm) return interaction.reply({ embeds: [new EmbedBuilder().setColor('#00E5FF').setDescription('Only the original host or an Admin can delete this giveaway.')], flags: MessageFlags.Ephemeral });
        ...
        if (subcommand === 'create') {
            const isHost = db.hasPerm(interaction.member, 'gwHosts') || interaction.user.id === config.ownerId || (interaction.member.permissions && interaction.member.permissions.has('Administrator'));
            if (!isHost) return interaction.reply({ embeds: [new EmbedBuilder().setColor('#00E5FF').setDescription('**Staff only!**')], flags: MessageFlags.Ephemeral });
